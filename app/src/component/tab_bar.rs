...
impl<T> Clone for TabBarCtx<T> {
    fn clone(&self) -> Self {
        Self { value: self.value.clone(), callback: self.callback.clone() }
    }
}


#[component]
pub fn TabBar<T: 'static>(value: ReadSignal<T>, on_select: Callback<T>, children: Element) -> Element {
    use_context_provider(|| TabBarCtx { value, callback: on_select });
    // provide_context();
